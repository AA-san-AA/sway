...
#include "wlr_hello_protocol.h"
struct wl_scheduler;
static struct wl_scheduler *scheduler = NULL;
// 回调函数类型
typedef void (*scheduler_result_func)(void *data,
        struct wl_scheduler *scheduler,
        uint32_t success,
        const char *message);

// 监听器结构
struct scheduler_listener {
    scheduler_result_func result;
};

void set_server_rr_scheduling(struct wl_display *display,uint32_t target_pid, int priority) {
    if (scheduler) {
        // 正确的请求发送方式
        wl_proxy_marshal((struct wl_proxy *)scheduler,
                WL_SCHEDULER_SET_RR, target_pid, priority);
        wl_display_flush(display);  // 确保请求被发送
        wl_display_roundtrip(display);  // 等待响应
    }
}

void print_scheduler_info(void) {
	wl_proxy_marshal((struct wl_proxy *)scheduler, 1);
}

static void handle_result(void *data,
        struct wl_scheduler *scheduler,
        uint32_t success,
        const char *message){
            printf("设置调度策略结果：%s (success: %d)\n", message, success);
        }
// 监听器实现
static const struct scheduler_listener scheduler_listener = {
...
static void handle_global(void *data, struct wl_registry *registry,
	...
	} else if (strcmp(interface, wl_scheduler_interface.name) == 0) {
		scheduler = wl_registry_bind(registry, name, 
			&wl_scheduler_interface, 1);
		wl_proxy_add_listener((struct wl_proxy *)scheduler, (void (**)(void))&scheduler_listener, NULL);
	}
}
